...
{
    sfColor colors[5] = {sfRed, sfBlue, sfYellow, sfGreen};
...
void init_champions(arena_t *arena)
{
    for (int i = 0; i < MAX_CHAMPIONS; i++) {
        ...
        arena->champions[i]->procs->carry = 1;
        arena->champions[i]->procs->wait = 0;
        arena->champions[i]->procs->next = NULL;
        arena->champions[i]->procs->pc_display =
            create_pc_display(arena->params, arena->champions[i]->color);
    }
}
