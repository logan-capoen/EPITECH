...
#include "../include_csfml/instructions.h"
#include "../include_csfml/tab.h"
...
{
    if (cycle_to_die != 0 && cycle_to_die % (CYCLE_TO_DIE - (CYCLE_DELTA * (arena->nb_live_signals / NBR_LIVE))) == 0)
...
int launch_corewar(arena_t *arena)
{
    ...
    create_buttons(arena->params);

    if (!args.arg_type_array)
        return 84;
    arena->args = &args;
    while (sfRenderWindow_isOpen(arena->params->window)) {
        while (sfRenderWindow_pollEvent(arena->params->window,
            &(arena->params->event)))
            analyse_events(arena);
        nb_cycle++;
        sfRenderWindow_clear(arena->params->window, sfBlack);
        if (arena->params->menu == 0) {
            display_vm(arena);
            display_pc(arena);
            display_cycle(arena, nb_cycle);
            display_champions(arena);
            check_cycle_to_die(arena, nb_cycle);
            browse_champs(arena, tmp_proc, &first_loop);
            if (is_one_standing(arena) && arena->params->menu == 0)
                break;
        } else {
            sfRenderWindow_drawSprite(arena->params->window, arena->params->background, NULL);
            sfRenderWindow_drawText(arena->params->window, arena->params->title, NULL);
            for (int i = 0; i < MAX_CHAMPS + 1; i++) {
                sfRenderWindow_drawRectangleShape(arena->params->window,
                arena->params->buttons[i]->rect, NULL);
                sfRenderWindow_drawText(arena->params->window, arena->params->buttons[i]->text, NULL);
            }
        }
        sfRenderWindow_display(arena->params->window);
    }
    for (; arena->alive_champions[last] == false || arena->alive_champions[last] == NULL; last++);
    if (!arena->alive_champions[last]) {
        my_printf("Nobody survives.\n");
        return 0;
    }
