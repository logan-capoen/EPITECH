...
{
    char *hex_str = malloc(sizeof(char) * 3);

    if (!hex_str)
        return NULL;
    snprintf(hex_str, 3, "%X", decimal);
    if (my_strlen(hex_str) == 1) {
        hex_str[1] = hex_str[0];
        hex_str[0] = '0';
    }
    hex_str[2] = '\0';
...
static int fill_vm(arena_t *arena, int i)
{
    ...
    for (int j = position; j < position + len_champion; j++) {
        ...
        sfText_destroy(arena->params->vm[j % MEM_SIZE]);
        arena->params->vm[j % MEM_SIZE] = create_text(arena->params, dec_to_hexa(instructions[index]), 15, arena->champions[i]->color);
        index++;
    }
    free(instructions);
    return 0;
