    ...
    champion_t *champion, proc_t *curr_proc)
{
    ...
    if (arg_type_array[0] == T_REG) {
        ...
        sfText_destroy(arena->params->vm[curr_proc->pc + param2 % IDX_MOD]);
        arena->params->vm[curr_proc->pc + param2 % IDX_MOD] =
            create_text(arena->params,
            dec_to_hexa(champion->registers[param1 - 1]), 10, champion->color);
    }
    return;
}
