...
static double computeAmbientOcclusion(const Scene &scene, const Intersection &intersection, bool ambient_occlusion_enabled)
{
    if (!ambient_occlusion_enabled || !scene.isAmbientOcclusionEnabled())
        return 1.0;
...
RenderConfig Renderer::buildRenderConfig(const Scene &scene, bool preview_mode) const
{
    if (preview_mode)
        return {5, 2, false, scene.getBackgroundColor()};
    return {scene.getSamplesPerPixel(), scene.getMaxRayDepth(), true, scene.getBackgroundColor()};
}
...
Color Renderer::traceRay(const RenderContext &context, const Ray &ray, int depth) const
{
    ...
    const double ambient_occlusion = (hit.material->isDelta()) ? 1.0 : computeAmbientOcclusion(context.scene, hit, context.config.ambient_occlusion_enabled);
    Ray scattered;
    Color attenuation;
...
{
    const RenderConfig preview_config = buildRenderConfig(scene, true);
    const RenderConfig final_config = buildRenderConfig(scene, false);
    Image preview_image = renderTiled(scene, camera, &sfml, preview_config);

    if (!sfml.isOpen())
        return preview_image;
    return renderTiled(scene, camera, &sfml, final_config, &preview_image);
}

Image Renderer::render(const Scene &scene, const Camera &camera) const
{
    return renderTiled(scene, camera, nullptr, buildRenderConfig(scene, false));
}

Image Renderer::renderTiled(const Scene &scene, const Camera &camera, Sfml *sfml, const RenderConfig &config,
    const Image *initial_image) const
{
    Image image = (initial_image == nullptr) ? Image(camera.getWidth(), camera.getHeight()) : *initial_image;
